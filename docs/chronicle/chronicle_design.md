...
| `active` | boolean | When true, the Entry is relevant to the current scene. |
...
2. If the viewer is a player controlling Creature C:
   - If `owner_id` is set and not equal to C, the Entry is invisible.
   - If C is in `hidden_from`, the Entry is invisible.
   - If `owner_id` is C, the Entry is visible.
   - Otherwise, the Entry is visible iff `public` is true.
