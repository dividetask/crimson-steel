...
| `skills` | list of string | no | Keys into `Casting Skills List`. **Default `[arcana]`** when omitted. Universal skills appended at lookup for Spells. |
...
| `item_only` | bool | spell only | Default `false`. Suppresses universal item form appending. |
| `item_only` on Talent | — | — | Always rejected. |
...
Presence of a Channel Block makes a Catalog Ability a Channeled Ability. The Combatant spends a Main Action on it each of their turns (the cast itself counts as the first such channel) or the spell ends.

| Field | Type | Default | Description |
|---|---|---|---|
| `mode` | enum | required | One of `fire`, `reservoir`, `maintain`, `auto`. See per-mode fields below. |
| `apply_on_cast` | bool | `false` | When `true`, the spell also fires its top-level effect on the initial cast in addition to whatever the channel does. Use for spells with a distinct cast-effect separate from the channel-effect (e.g. Heal heals damage on cast in addition to reducing bleeding on each channel). |
...
| `attack_roll` | bool | `false` | When `true`, the channel resolves as an attack roll using the channeled dice. Independent of the top-level `attack_roll`. (fire-mode only.) |
| `save` | list of Save Spec | empty | Save Specs evaluated against the channel effect. Independent of the top-level `save`. (fire-mode only.) |
| `effect_hash` | dict | empty | Channel-effect values. Namespaced separately from the top-level `effect_hash`. |
| `reservoir_ratio` | int | `1` | Reservoir-mode and auto-mode only. Reservoir dice added per channeled die. |
| `activation` | Activation | required for reservoir | Reservoir-mode only. How a single Reservoir die is spent to trigger the spell. See below. |

**Activation** (reservoir mode):

| Field | Type | Default | Description |
|---|---|---|---|
| `action` | enum | required | Action category for triggering the spell — `bonus`, `reaction`, or `free`. (Main-action triggers are handled by channeling itself, not by Activation.) |
| `description` | string | absent | What the triggered spell does. |
| `attack_roll` | bool | `false` | Whether the triggered effect resolves as an attack roll. |
| `save` | list of Save Spec | empty | Saves evaluated on the triggered effect. |
| `effect_hash` | dict | empty | Activation-effect values. |

Each Activation always costs exactly one Reservoir die.

**Channel Mode notes:**

- **fire**: each turn's channel fuels the spell's effect directly. The channeled dice serve as the Roll (when `attack_roll` or a Save is involved) or drive the effect formula (e.g. Heal's bleed reduction scales with successes on the casting check). Channel dice are 4 to Combat Pool Remaining; Dice Cap does not apply to channeling.
- **reservoir**: each turn's channel adds dice to a persistent Reservoir at `reservoir_ratio`. The Reservoir is spent later via Activations. Channel dice are 4 to Combat Pool Remaining.
...
- **auto**: the cast itself fills the Reservoir at `reservoir_ratio`. After cast, the spell self-sustains — no Main Action channel is required each turn. The Reservoir is not spent by Activations; instead it defines the per-turn effect's magnitude (e.g. Spiritual Weapon makes one attack per turn using a number of dice equal to its Reservoir). The spell ends when its declared `duration` expires. Spell text describes any optional actions (e.g. spending a Main Action to redirect the auto effect).

### Area

| Field | Type | Description |
|---|---|---|
| `shape` | enum | `line`, `cone`, `square`, `circle`. *(configurable)* |
| `size` | non-neg int | In 5-foot squares. Never feet. |
| `anchor` | enum | Where the area is centered. Default `point` (caster designates a point at cast time). Other values: `target` (centered on and moves with the Ability's named target), `caster` (centered on and moves with the caster). |
...
| `condition` | string | no | Free-form scope tag — `target_flatfooted`, `target_undead`, etc. |
...
| Field | Type | Description |
|---|---|---|
| `kind` | string | Outcome shape. Recognized today: `bonus_dice`, `scale`, `ui_reveal`, `invoke_named_effect`. |
