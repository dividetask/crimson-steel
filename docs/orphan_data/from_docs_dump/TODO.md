...
- [ ] **Timekeeping module** (`lib/timekeeping.rb`, `docs/timekeeping/`).
  New module owning Round counter, Tick counter (within current Round), Ticks Per Round divisor, Game Day counter, and time-of-day. API: `start_combat(ticks_per_round)`, `advance_tick`, `end_combat`, `current_tick`, `current_round`, `current_day`, plus an `advance_time(rounds:)` for out-of-combat passage. Combat is the primary caller; downtime and `Advance Time` operations call `advance_time` and the equipment Game Day API moves into Timekeeping.

- [ ] **Combat tick scheduler** (refactor inside `lib/combat.rb`).
  Replace today's "round counter + turn pointer wraps once per round" model with the per-Combatant Tick Schedule documented in `combat_design.md`. Compute Tick Schedule at `add_combatant` and recompute when Ticks Per Round changes. Add `acting_combatants_at(tick)`. Drop the `round` field from `combat_data.yaml`; defer to Timekeeping. Combat must call `Timekeeping.start_combat(max_turns_per_round)` at combat start and `Timekeeping.advance_tick` on each tick advance.

- [ ] **Non-divisible Turns Per Round handling.**
  Today's Tick Schedule formula assumes `Turns Per Round[tier]` divides cleanly into Ticks Per Round (true for the default `[1,1,1,2,4,8]`). Mixes that don't divide cleanly (e.g., a 3-turn Combatant alongside a 5-turn Combatant) need explicit LCM-based handling. Decide whether to grow Ticks Per Round to LCM, or pick a different distribution rule. Defer until non-power-of-2 Turns Per Round values become a real config.
