...
The Combat module is a thin state machine: it owns the **persisted state** of one in-progress Combat (combatants, initiative, per-Combatant Tick Schedules) and computes per-Combatant derived values on demand by looking up the Character through a callback. Round and Tick counters live on **Timekeeping** (see common glossary); Combat reads Timekeeping's current Tick to determine which Combatants act and notifies Timekeeping when a Tick completes.

## Implementation language

The operations described below are split between server-side Ruby (`lib/combat.rb`) and client-side JavaScript (`public/js/attack_stub.js`). The split today:

- **Ruby (`Combat`)** owns: state persistence (`combat_data.yaml`), Combatant management (Combat ID allocation, `add_combatant`, `remove_combatant`), Tick Schedule computation (`tick_schedule_for`, recomputed when Ticks Per Round changes), Acting Combatants lookup (`acting_combatants_at(tick)`), Timekeeping notifications at combat start/end and on tick advance, Combat Pool computation and spend (`combat_pool`, `spend_combat_pool`, `reset_combat_pool`), initiative rolling and reroll with combat-specific Luck and Insight (`reroll_all_initiative`), `apply_attack_damage` (the Severity Calculation half of attack resolution including Runtime Bucketing and side-effect routing), and `critical_modifier_for(damage_type)`.
- **JavaScript (`attack_stub.js`)** owns: the DM-facing attack flow UI — target/weapon/defense/ally-reaction selection, per-step preview Target Number calculation (`attackTn`, `defenseTn`), Luck assignment table, Damage form. It dispatches an `attack:confirm` event with the chosen payload; the host page is responsible for handing that off to Ruby (`Combat#apply_attack_damage` for damage routing). The JavaScript layer never performs dice math itself — it embeds `multi_roll_stub` (which talks back to Ruby's `DiceSystem`) for actual Rolls.

The "either half may be implemented in either language" remark in the Attack Resolution Pipeline section below is the only attack-related operation whose language allocation is genuinely fluid.
...
Each Combatant gets a precomputed **Tick Schedule** — the list of Ticks within a Round on which that Combatant acts. Computed at `add_combatant` and recomputed whenever Ticks Per Round changes (a Combatant joining or leaving may change the max `Turns Per Round[tier]` across present Combatants).

Formula (per Timekeeping / common glossary): for `i = 1..Turns Per Round[tier]`, `tick_i = floor((Ticks Per Round * (2i - 1) + Turns Per Round) / (2 * Turns Per Round))` — the floored midpoint of the i-th equal segment of the Round. Worked example with Ticks Per Round = 4 and three Combatants (Tier 0/1/2 → 1 turn, Tier 3 → 2 turns, Tier 4 → 4 turns):

- 1-turn Combatant: tick **2** (middle of round).
- 2-turn Combatant: ticks **1, 3** (segment midpoints).
- 4-turn Combatant: ticks **1, 2, 3, 4** (every tick).

`acting_combatants_at(tick)` returns every Combatant whose Tick Schedule contains the given Tick, sorted by Initiative String descending then Combat ID. `advance_tick` calls `Timekeeping.advance_tick` and, when the Tick wraps past `Ticks Per Round - 1`, Timekeeping rolls Round forward; Combat does not track Round itself.

**Constraint**: today's design assumes every Combatant's `Turns Per Round[tier]` divides cleanly into Ticks Per Round. Mixes that don't divide cleanly (e.g., a 3-turn Combatant alongside a 5-turn Combatant) need explicit LCM handling that isn't implemented yet — see TODO.
...
- The single in-memory Combat: combatants, per-Combatant Tick Schedule, active flag.
- Combat ID allocation and the two-ID identity scheme.
- Turn ordering via Initiative String compare (within a Tick).
- Tick Schedule computation per Combatant (recomputed when Ticks Per Round changes).
- Acting Combatants lookup by Tick.
- Timekeeping notifications: `start_combat(ticks_per_round)`, `advance_tick`, `end_combat`.
...
- **Round and Tick counters, time-of-day, Game Day** — Timekeeping owns those. Combat asks Timekeeping for the current Tick and notifies Timekeeping on tick advances; Timekeeping rolls Round when Tick cycles.
...
- **Non-divisible Turns Per Round mixes.** When a Combatant's `Turns Per Round[tier]` doesn't divide cleanly into the active Ticks Per Round (e.g., 3 vs 5), the floor-midpoint formula produces unevenly-spaced ticks. Needs explicit LCM-based handling — see TODO.
