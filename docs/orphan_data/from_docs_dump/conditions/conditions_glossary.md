...
(Magic Toxicity, Toxicity Threshold, Acceptance Check, Toxicity Poisoning: see common glossary.)

## Death

**Death Threshold**: A Character is Dead when any of these is reached:
- **HP**: total damage ≥ `Death Multiplier × max_hit_points`.
- **Attribute**: total ability damage to a single attribute ≥ `Death Multiplier × attribute_score`.
- **Toxicity**: Magic Toxicity ≥ `Death Multiplier × Toxicity Threshold`.

*(configurable: `Death Multiplier`, default 2.)* Exposed via `Conditions#dead?(character)`; `max_hit_points`, attribute scores, and the Toxicity Threshold are read back through the Character. See **Dead** in the common glossary for the cross-module concept.
