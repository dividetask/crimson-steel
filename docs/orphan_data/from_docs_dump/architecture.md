...
| Proficiency | `docs/skills/` | `lib/proficiency.rb` | ✅ | Skill catalog + coordinator |
...
| Timekeeping | `docs/timekeeping/` | `lib/timekeeping.rb` (planned) | — | game clock — Round / Tick / Day; Combat is the primary advancer |
...
- **Proficiency is a thin coordinator.** Resolves Skills (with Set-prefix fallback), computes Skill Prowess, folds in the configured Competency adjustments, routes Versatile Performance. Per-Character Ranks still live on Advancement.
...
**Proficiency** — Skill catalog plus a coordinator class exposing `skill_details(skill_name, character, advancement)`. Composes `Skill Prowess = Ranks + floor(Attribute / divisor)`, partitions via `DiceSystem#compute_check_details`, and folds `competency_bonus_base` (and `untrained_competency_modifier` when ranks == 0) into the returned Competency value. Versatile Performance routed at lookup time. Plus the unenforced `minimum_skills_trained` directive.
...
| `data/skills.yaml` | Proficiency + Advancement | force-added |
...
### Proficiency owns the Skill catalog and the Competency math (decided)

`lib/proficiency.rb` is a thin coordinator with three jobs: Skill resolution (with Set-prefix fallback), Skill Prowess composition + Competency adjustment, and Versatile Performance routing. `DiceSystem#compute_check_details(prowess)` partitions Prowess into Dice Count + Competency Bonus + Starting Value, and Proficiency layers `competency_bonus_base` plus the optional `untrained_competency_modifier` on top before returning. Kills the "Skill-Roll API" and "validate skill list entries" Unassigned items.
