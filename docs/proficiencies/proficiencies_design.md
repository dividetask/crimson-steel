...
Rank accumulation across class levels, Class skill lists, and the rule that every Class trains saves are not Proficiencies concerns; those live in Creatures/Advancement.
...
5. **Proficiency Prowess.** `max(Direct Prowess, Substituted Prowess)`. Direct Prowess wins ties.
6. **Translate.** Call dice resolution's *Translate Skill Prowess into Roll inputs* with the Proficiency Prowess.
7. **Return.** `{dice_cap, competency_modifier}`. The modifier is `("Competency", bonus_penalty)` when `bonus_penalty != 0`; otherwise null.
...
- `Direct Prowess = effective_ranks + floor(creature.attribute_value(driving_attribute) / Attribute Contribution Divisor) + Non-Proficiency Penalty (when effective_ranks == 0)`.
...
- Otherwise, for each source key in the map whose target list contains the queried key:
  ...
  - `source_prowess = source_ranks + floor(creature.attribute_value(source_attribute) / Attribute Contribution Divisor) + Non-Proficiency Penalty (when source_ranks == 0)`.
- `Substituted Prowess = max(source_prowess across all matching sources)`.
