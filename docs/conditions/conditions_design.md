...
- **Save Roll mechanics.** Conditions calls Dice Resolution; how the Creature's resistance translates into `dice_count` and `modifiers` is the caller's job. The Save Input shape is the contract.
