...
- **Ascendancy (the Check half) is computed in the JavaScript Roll/Check Resolution engine.** A Roll carries its Creature's `tier` only on the combat builders; `CheckResolution.prepare` runs the Tier Mismatch step (`public/js/tierMismatch.js`) *after* cross-side Propagation, appending each Roll's Ascendancy entry — derived from its `tier` versus the opposing primary Roll's `tier` — to its `bonus_penalty_list`. Running after Propagation keeps the Ascendancy from being inverted back onto the other side. The attack builder and the cast builder stamp the attacker/caster Roll's Tier up front and the defender/target Roll's Tier when a Target is chosen; the skill-check builders leave `tier` unset, so a Roll with no `tier` (or no opposing Tier — e.g. a self/ally buff with no Defensive Roll) gets no Ascendancy. No extra round trip is needed.
- **Inherent damage reduction (the damage half) stays server-side**, since damage is applied server-side. `Encounter::TierMismatch.inherent_damage_reduction` is subtracted from the dealt damage in `Encounter::State#resolve_attack_payload` (reported as `inherent_dr`).
