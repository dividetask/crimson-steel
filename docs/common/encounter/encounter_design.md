...
**No-roll casts skip the roll and Luck.** The cast builder only asks the caster to roll a casting check when its Successes matter — a Save, attack-roll, or damage Spell. A reservoir-channel Spell (Shield of Faith) still asks for a dice count, but those dice are **poured into the Reservoir** at cast (`register_cast_sustain`'s `channel_dice`), not rolled. Every other Spell (buffs, no-save areas) casts at the **minimum** dice with no roll. For any no-roll Spell the builder skips the Luck steps and hides *Roll All*, so the DM just confirms. The builder also **hides Spells that take a minute or longer to cast** — they aren't cast in the heat of combat.
