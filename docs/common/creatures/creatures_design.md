...
| `skill_table` | list of skill keys | `[]` | Optional candidate skill list. At spawn the list is shuffled and the first **N** are assigned to the spawn's (single) class, where N is the Skill Pick Budget — `Skill Pick Formula` (`floor(int/4) + bonus_skills`) per Class Level, evaluated against the spawn's resolved Effective Intelligence and the rolled class's `bonus_skills`. Entries may be Set Instances (`perform_percussion`) but not bare Set Skill keys. |
