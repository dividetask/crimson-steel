...
2. **Weapon & dice.** Each equipped weapon (from the *Attack Options* fetch, which carries the weapon's martial Dice Cap and Speed) renders as a row: the weapon name plus a strip of dice-count buttons from `2` up to the weapon's Dice Cap. Clicking the weapon name picks the **largest affordable** roll — `min(Dice Cap, floor(Combat Pool Remaining / Speed))`; clicking a specific number picks that many dice. Dice counts the Combat Pool cannot afford (`dice × Speed > remaining`) are disabled.
...
4. **Roll.** The attacker's Roll (and the declared defender's Opposing Roll — Dodge at full Dice Cap, Parry/Block at the largest the defender's pool affords) render as a **Check Resolution Stub** (`check_resolution_stub.md`) — the exact same Roll UI and engine the Dice / Check stubs use. The Bonus/Penalty sources (Competency, Flatfooted, Unaware) are folded into each Roll's TN + Starting Value server-side via `DiceResolution.compute_target_number`. The DM rolls it (`Roll All`), may reroll/nudge, and may type a manual override into the Result field (the standard Check-stub affordance).
5. **Resolve.** A single POST carries the Confirm payload (below), reading each Roll's resolved Successes (and the defender's dice count) out of the Check stub. The server spends Combat Pool for every participant, nets Supporting minus Opposing Successes per *Check Resolution*, and on a positive net Degree of Success routes the damage through *Apply Damage* → Conditions' *Apply Hit Point Damage*. Temporary HP absorbs worst-first (Major → Moderate → Minor) before the Severity counters increment, per `conditions_design.md`.
...
- **Implemented:** the panel (`views/_turn_action.erb`, rendered by `views/encounter.erb`; menu switching + Attack flow in `public/js/ui/turnAttack.js`).
  ...
  - **Attack** — the progressive-disclosure flow above, driving `GET /encounter/attack_options` (weapons with Dice Cap + base damage, and targets), `POST /encounter/attack_check` (renders the Check Resolution Stub for the attacker's Roll plus any declared defender's Opposing Roll), and `POST /encounter/resolve_attack`. Attacker Bonuses (Flatfooted, Unaware) and Defensive Actions (Parry / Block / Dodge) are assembled server-side; the dice are resolved by the shared Check engine.
