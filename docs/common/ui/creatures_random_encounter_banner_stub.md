# Creatures Random Encounter Banner Stub

A banner shown after a Random Encounter has been rolled, describing the Creatures that were just added to the active Combat and any loot rolled alongside them. Renders above the main content on pages that surface combat or roster state.
...
1. **Title** — the Random Encounter's name as supplied by the parent page.
2. **Outcome description** — body text describing the Random Encounter outcome. Uses the show-more truncation pattern (per `ui_conventions.md`).
...
4. **Dismiss button** — a `×` button on the right. DM-only. Emits a `dismiss_banner` event the parent page resolves by clearing the Random Encounter message from its scene state.

## Parameters

Required:
- Random Encounter Name — string. Supplied by the parent page.
...
- Loot list — the Item Stacks produced by Equipment's *Roll Loot Table* call that accompanied the Random Encounter (may be empty).
- Viewer role — `dm` or `player`. Determines whether the Dismiss button renders.

## When it renders

The parent page renders the banner only when a Random Encounter has been rolled since the last dismissal. The Random Encounter message is scene-level state owned by the parent page; this stub does not persist it.
...
The banner is a partial included by parent pages above their main content when a Random Encounter is active. Not embedded inside other stubs.

## What this stub does not do

- It does not roll Random Encounters. Random Encounter rolling is the parent page's responsibility — typically via Creatures' *Roll Random Encounter* (which spawns the new Creature records via *Spawn Creature From Template* and returns their IDs); the parent then adds each via Combat's *Add Combatant* and may roll accompanying loot via Equipment's *Roll Loot Table*.
