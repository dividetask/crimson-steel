...
3. **Initiative · Combat Pool · Perception · Speed row** (full width, four equal columns)
   ...
   - `Combat Pool <size>` — the Creature's **maximum** spendable dice on one of its turns, as Combat computes it (`encounter_design.md` → Operations → *Combat Pool computation*). This is the full pool size, never the remaining pool: the sheet is read outside Combat, and the spent/remaining split belongs to the initiative stub. No `d` suffix. Shown for every Creature the sheet renders — player characters, NPCs, and enemies alike.
     - **Combat Pool popup.** Hovering the cell, keyboard-focusing it, or clicking/tapping it reveals a popup anchored to that cell — the same reveal / click-toggle machinery as the Attribute and Damage Reduction popups, and subject to the same one-popup-at-a-time rule. The popup shows the computation's **two stages** on their own lines rather than one summed breakdown line, because the second stage is a purchase and not an addition:
       - `Budget (<martial ranks> martial × 2 + <value> <ATTR>) ÷ <turns> turns per round = <budget>` — `<ATTR>` is the configured Combat Pool Attribute, and `<turns>` the Turns Per Round for the Creature's Tier.
       - `Pool budget <budget> buys <size> (cost <cost>) = <size>` — the tiered Buy, where each successive block of `Combat Pool Step` points costs one more Budget point each. The first `Combat Pool Step` points are free, so this line covers the guaranteed minimum too: a Creature with a Budget of 0 still reads as buying `Combat Pool Step` points at a cost of 0.
       Encounter computes every number — Budget, size, and cost; the stub only formats them.
...
- **Encounter domain (Combat Pool)** — the Combat Pool size, together with the Budget-stage inputs (martial ranks, the Combat Pool Attribute and its Effective value, the Tier's Turns Per Round) and the Buy-stage values (`Combat Pool Step` and the size's cost) shown in the Combat Pool popup. The stub never recomputes the formula.
