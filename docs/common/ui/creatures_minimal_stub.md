...
2. **Attribute + vital block** (full width, three columns side by side)
   - **Left column**: 3×2 grid of attributes. Each cell shows the attribute key in small-caps maroon (`STR`, `DEX`, ...) followed by the Effective Attribute value. Order is the project's `Attributes` config order — typically str, dex, con on the first row; int, wis, cha on the second.
     ...
     - **Attribute popup.** Hovering over an attribute cell, keyboard-focusing it, or clicking/tapping it reveals a small popup anchored to that cell. The popup lists the Ranks, Dice Cap, and Bonus for that Attribute's **Check** and its **Save** (the Wisdom save, Dexterity save, etc.). When the Creature has the Floor Ability (Jack of All Trades), the popup also lists an **Untrained** row — the Ranks, Dice Cap, and Bonus an untrained (zero-rank, non-Restricted) Skill driven by that Attribute would roll, reflecting the Floor lift. The Untrained row is omitted for Creatures without the Floor Ability. The Ranks column shows the effective ranks behind each row: an Attribute Check uses no Skill ranks so its Ranks cell is always `0`; the Save row shows the Creature's ranks in that Attribute's Saving Throw proficiency; the Untrained row shows the ranks the Floor Ability grants (`floor(level / 2)`). All values are requested from Proficiencies / Dice Resolution; the stub never recomputes them. Save bonuses are **broken out** rather than baked in: after the Competency, each applicable Save bonus is appended as its own signed token on the same line — green for a bonus, red for a penalty — so a Creature with a `+1` Cloak of Resistance and a conditional `+1` poison resistance reads `+1+1*+1` (Competency, Cloak, then the conditional). **Conditional** bonuses (racial resistance vs poison / enchantment, fey resistance vs charm) carry a leading `*` to mark that they only apply against their descriptor. **Inherent** bonuses are the exception — they stay baked into the underlying value rather than shown as a token. Only one Attribute popup is visible at a time — clicking toggles a sticky popup so it works on touch, and hovering another attribute dismisses it; hover and focus reveal a popup transiently.
...
- **Proficiencies domain** — Perception's Competency Modifier; the per-Attribute Check / Save Dice Caps and Bonuses and the untrained-Skill (Floor Ability) Dice Cap and Bonus shown in the Attribute popup.
