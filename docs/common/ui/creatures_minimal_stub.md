...
3. **Initiative · Combat Pool · Perception · Speed row** (full width, four equal columns)
   ...
   - `Combat Pool <size>` — the Creature's **maximum** spendable dice on one of its turns, as Combat computes it (`encounter_design.md` → Operations → *Combat Pool computation*). This is the full pool size, never the remaining pool: the sheet is read outside Combat, and the spent/remaining split belongs to the initiative stub. No `d` suffix. Shown for every Creature the sheet renders — player characters, NPCs, and enemies alike.
     - **Combat Pool popup.** Hovering the cell, keyboard-focusing it, or clicking/tapping it reveals a popup anchored to that cell — the same reveal / click-toggle machinery as the Attribute and Damage Reduction popups, and subject to the same one-popup-at-a-time rule. The popup shows the computation's **two stages**, each in the shape of the arithmetic it actually performs: Stage 1 is a sum, so it reads as a breakdown line; Stage 2 is a purchase, so it reads as a price table. A breakdown line cannot carry Stage 2 — a lone `cost <n>` states a total without showing where it came from, and leaves the reader unable to see why the Pool stopped at the size it did.
       ...
       - **Buy table** — a real table with columns `Dice`, `Each`, `Cost`, ruled between rows. One row per block of `Combat Pool Step` dice: the first block reads `free` (points 1..Step cost nothing), and each later block costs one Budget point more per die than the one before. A block's `Dice` cell reads as a range (`5–8`), or as a bare number when the block holds a single die; it is the row's label, so it is left-aligned and given enough width to keep the two numeric columns aligned. The **final block is partial** whenever the Pool stops mid-block, and shows only the dice actually bought (`13–13`, rendered `13`). A closing total row reads `<size> dice` against `<cost> of <budget>`, set off above by a heavier rule and carrying the table's only bold figures — the row the eye should land on. The row rules matter: without them the price blocks read as loose columns of text rather than as a table.
       - **Stop line** — `Next die costs <price> — <remaining> budget left`, below the table. This is what makes the size legible: the Buy stopped because the next die's price exceeds what the Budget has left. `<price>` is `floor(size / Step)`, the cost of the die after the last one bought.
...
- **Encounter domain (Combat Pool)** — the Combat Pool size, together with the Budget stage's inputs and running values (martial ranks, the Combat Pool Attribute and its Effective value, the sum before the division, the Tier's Turns Per Round) and the Buy stage's price blocks (each block's dice range, per-die cost, and amount spent), the total cost, the Budget left over, and the next die's price. The stub never recomputes the formula.
