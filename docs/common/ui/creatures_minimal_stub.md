...
3. **Initiative · Combat Pool · Perception · Speed row** (full width, four equal columns)
   - `Initiative <dice_count>` — the dice count Combat would roll on a fresh initiative roll. No `d` suffix.
   - `Combat Pool <size>` — the Creature's **maximum** spendable dice on one of its turns, as Combat computes it (`encounter_design.md` → Operations → *Combat Pool computation*). This is the full pool size, never the remaining pool: the sheet is read outside Combat, and the spent/remaining split belongs to the initiative stub. No `d` suffix. Shown for every Creature the sheet renders — player characters, NPCs, and enemies alike.
     - **Combat Pool popup.** Hovering the cell, keyboard-focusing it, or clicking/tapping it reveals a popup anchored to that cell — the same reveal / click-toggle machinery as the Attribute and Damage Reduction popups, and subject to the same one-popup-at-a-time rule. The popup shows the computation's **two stages**, each in the shape of the arithmetic it actually performs: Stage 1 is a sum, so it reads as a breakdown line; Stage 2 is a purchase, so it reads as a price table. A breakdown line cannot carry Stage 2 — a lone `cost <n>` states a total without showing where it came from, and leaves the reader unable to see why the Pool stopped at the size it did.
       - **Budget lines** — two short lines, one per step of the formula, rather than one line carrying the whole thing. The first is the sum, `<martial ranks> martial × 2 + <value> <ATTR> = <sum>`; the second divides it, `<sum> ÷ <turns> turn(s) per round = <budget> Budget`. Splitting them keeps each line inside the popup at realistic values — a single combined line wraps, and a wrapped formula is harder to read than two whole ones. `<ATTR>` is the configured Combat Pool Attribute and its **Effective** value; `<turns>` is the Turns Per Round for the Creature's Tier. The division is shown even when Turns Per Round is 1, so the formula reads the same on every sheet and the Tier's role stays visible. "turn" is singular at 1, plural otherwise.
       - **Buy table** — a real table with columns `Dice`, `Each`, `Cost`, `Left`, ruled between rows. One row per block of `Combat Pool Step` dice: the first block reads `free` (points 1..Step cost nothing), and each later block costs one Budget point more per die than the one before. A block's `Dice` cell reads as a range (`5–8`), or as a bare number when the block holds a single die; it is the row's label, so it is left-aligned and given enough width to keep the three numeric columns aligned. `Left` is the Budget **after** that block is paid for, so the column runs the Budget down to whatever the Buy had in hand when it stopped — the first row's `Left` is therefore the full Budget whenever the free block leads. The **final block is partial** whenever the Pool stops mid-block, and shows only the dice actually bought (`13–13`, rendered `13`). A closing total row reads `<size> dice`, the total `<cost>`, and the `<remaining>` Budget, set off above by a heavier rule and carrying the table's only bold figures — the row the eye should land on. The row rules matter: without them the price blocks read as loose columns of text rather than as a table.
       - **Stop line** — `Next die costs <price>`, below the table. This is what makes the size legible: the Buy stopped because the next die's price exceeds the `Left` figure the table ends on. `<price>` is `floor(size / Step)`, the cost of the die after the last one bought. It names only the price, never the remainder — the `Left` column already carries that, directly above it.
       The first `Combat Pool Step` dice being free is also what makes the guaranteed minimum automatic — a Creature with a Budget of 0 shows a single `1–4 · free · 0` row and a `4 dice · 0 of 0` total, with no special-casing. Encounter computes every number — Budget, size, per-block costs, and the next die's price; the stub only formats them.
...
Below the Initiative · Combat Pool · Perception · Speed row the sheet splits into two columns. The split runs through the Actions/Skills (left) and Spells/Rituals/Items (right) sections; Abilities and Item Descriptions render in their own two-column row below.
...
- **Equipment + Conditions domains** — Damage Reduction and Damage Resilience (equipped Armor totals plus active-effect Modifiers), and the per-Armor-piece contributions — each piece's Reduction and its Resilience `tier × increment` factors — that fill the math popup. Damage Resilience additionally carries the Creature's **Tier** as a flat base component (labelled `tier` in the popup; Tier 0 contributes 0), so its total is `Tier + Armor Resilience + Resilience Modifiers`. Damage Reduction does not include the Tier.
...
- **Encounter domain (Combat Pool)** — the Combat Pool size, together with the Budget stage's inputs and running values (martial ranks, the Combat Pool Attribute and its Effective value, the sum before the division, the Tier's Turns Per Round) and the Buy stage's price blocks (each block's dice range, per-die cost, amount spent, and the Budget left after it), the total cost, the Budget left over, and the next die's price. The stub never recomputes the formula.
