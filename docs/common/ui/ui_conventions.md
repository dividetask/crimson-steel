...
Any spec that displays a free-form text body uses the show-more pattern: text up to the first 200 characters is always visible; anything beyond is hidden behind an inline `Show more` affordance that expands the body in place. Newlines within the visible window render as line breaks. Specs that need a different threshold state the override explicitly; otherwise 200 characters is the convention.
...
Stubs may persist UI-only preferences in the browser's `localStorage` so a reload restores the user's last view. Persistence is per-browser and never authoritative; the application's data is always the source of truth, and a missing or stale localStorage value falls back to the spec's default. Conventional keys used by stubs in this folder:

- Character-sheet detail mode (`full` vs `minimal`) — keyed by Creature ID.
- Section collapse state for stubs that expose collapsible blocks — keyed by section label or by the parent context.
- Scene-level toggles that are unique to a single host page — a global key for that page is acceptable.

A stub that uses localStorage names its key in its own spec; this section catalogs the convention so multiple stubs do not collide on the same key name.
