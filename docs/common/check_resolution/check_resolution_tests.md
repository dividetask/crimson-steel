...
**Spread preparation is bidirectional.** A `spread` Check with a caster carrying `['Inherent', 2]` and an Opposer carrying `['Competency', 3], ['Inherent', 1]`: propagation produces caster `[['Inherent', 2], ['Competency', -3], ['Inherent', -1]]` and Opposer `[['Competency', 3], ['Inherent', 1], ['Inherent', -2]]`. TN computation then derives each Roll's Ascendancy from its Inherent imbalance — caster `['Ascendancy', 2]` (the one-point surplus amplified), Opposer `['Ascendancy', -2]` — which `previewParameters` surfaces on each `bonusPenaltyList`.
