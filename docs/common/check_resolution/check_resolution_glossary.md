...
- **Ascendancy** — The derived modifier that amplifies an Inherent imbalance, derived per Roll during TN computation (Roll Resolution), **not** by Check Resolution. Gated on a present Inherent **Penalty** (value ≤ 0, a 0 counts): a lone Inherent Bonus, or no Inherent entries at all, derives nothing. On a combat Check, propagation supplies the opponent's Inherent as the Penalty; a one-sided Roll (an Affliction save) carries it directly. See `../dice_resolution/dice_resolution_design.md` → *Ascendancy*.
