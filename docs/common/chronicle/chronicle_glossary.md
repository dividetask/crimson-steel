...
**Shared**: A visibility marker on an Entry. When set, every player sees the Entry. When unset, only the owner (plus the Game Master) sees it; a DM-created Entry with no owner is GM-only when unset.

**Hidden From**: A per-Entry list of Creatures from which the Entry is hidden. A player controlling any listed Creature cannot see the Entry even when it is Shared.

**Owner**: The player Creature that created an Entry. Absent when the Entry was created by the Game Master.
...
**Active**: A marker that places an Entry in the Scene. Inactive Entries are archived — they record events that have already happened or information not currently relevant.
