...
**A trained Skill with a middling attribute.** Given `key = "athletics"`, `ranks_for("athletics") = 3`, `attribute_value("str") = 4`, no relevant abilities. Direct Prowess = `3 + floor(4 / 2) = 5`. No Substituted Prowess. Proficiency Prowess = 5. Dice resolution translates Prowess 5 to `dice_cap = 6, bonus_penalty = 1`. Trained, so no Non-Proficiency Penalty. Returned: `{dice_cap: 6, competency_modifier: ("Competency", +1)}`.

**An untrained Skill applies the Non-Proficiency Penalty on the Target Number, not the dice.** Given `key = "stealth"`, `ranks_for("stealth") = 0`, `attribute_value("dex") = 3`, no relevant abilities. Prowess = `0 + floor(3 / 2) = 1` (ranks + attribute only — the penalty is never folded in). Dice resolution translates Prowess 1 to `dice_cap = 7, bonus_penalty = 0`; the untrained Skill then adds the `-2` Non-Proficiency Penalty to the Competency. Returned: `{dice_cap: 7, competency_modifier: ("Competency", -2)}`.
...
**A Set Instance with no ranks gets the Non-Proficiency Penalty.** Given `key = "perform_dance"`, `ranks_for("perform_dance") = 0`, `attribute_value("cha") = 2`. Prefix Match resolves to `perform_`, attribute `cha`. Prowess = `0 + floor(2 / 2) = 1` → `dice_cap = 7`; the untrained Skill adds `-2` to the Competency (a Target-Number penalty, not dice).
...
**Floor does not apply to Restricted Skills.** Given `key = "restricted_magic"`, `ranks_for("restricted_magic") = 0`, the creature has `jack_of_all_trades`, `level_for_ability("jack_of_all_trades") = 5`, `attribute_value("int") = 2`. Floor lift = 0 (Skill is Restricted), so effective ranks = 0. Prowess = `0 + floor(2 / 2) = 1` → `dice_cap = 7`; being an untrained Skill it adds `-2` to the Competency. Returned: `{dice_cap: 7, competency_modifier: ("Competency", -2)}`.

**Floor does not apply to keys without a catalog entry.** Given `key = "con_save"`, `attribute_override = "con"`, `ranks_for("con_save") = 0`, the creature has `jack_of_all_trades`, `level_for_ability("jack_of_all_trades") = 8`, `attribute_value("con") = 8`. Floor lift = 0 (no catalog entry), so effective ranks = 0 — but a Saving Throw **never** takes the Non-Proficiency Penalty. Prowess = `0 + floor(8 / 2) = 4` → `dice_cap = 10, competency_modifier: null`.
...
**Substitution wins when its Prowess exceeds Direct Prowess.** Given `key = "deception"`, `ranks_for("deception") = 0`, `ranks_for("perform_act") = 4`, the creature has `versatile_performance`, `attribute_value("cha") = 6`. Direct Prowess = `0 + 3 = 3` (untrained). Substituted Prowess from `perform_act`: `4 + 3 = 7` (trained). Substitution wins; Proficiency Prowess = 7, and because the winning source is trained no Non-Proficiency Penalty applies.

**Substitution attribute can differ from the queried key's.** Given `key = "acrobatics"` (target attribute `dex`), `ranks_for("acrobatics") = 0`, `ranks_for("perform_dance") = 4`, the creature has `versatile_performance`, `attribute_value("dex") = 0`, `attribute_value("cha") = 6`. Direct Prowess = `0 + 0 = 0` (untrained). Substituted Prowess: `4 + 3 = 7` (trained). Substitution wins.
