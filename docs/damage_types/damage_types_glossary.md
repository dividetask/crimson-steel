...
- **`counter`** — applies or increments a named counter on the target, with a turn-start behavior. Fields: `counter_name` (string), `on_turn_start` (a list of nested mechanics describing what happens at the start of the affected creature's turn). Acid's counter is `{kind: counter, counter_name: acid, on_turn_start: [...]}`. Consumed by the conditions module.
...
- **`inflict`** — inflicts a named condition by N points per damage point applied. Fields: `condition_name` (string), `per_damage` (integer). Cold's "two points of shock per point of damage" is `{kind: inflict, condition_name: shock, per_damage: 2}`. Consumed by the conditions module.
...
- **`scale_self`** — only valid inside `on_turn_start`. Multiplies a counter's value by a factor with a rounding rule. Fields: `factor` (number), `rounding` (`floor` or `ceil`).
- **`deal_damage`** — only valid inside `on_turn_start`. Deals damage from the counter's current value. Fields: `amount_formula` (Formula referencing the counter's value as `self`), `severity` (Severity name).
