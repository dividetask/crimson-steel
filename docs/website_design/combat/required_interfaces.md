...
stub needs. Each section is the **partial stub** that domain must provide for
combat to work.
...
per-Check math to it. Player-facing definitions live in the Compendium's Check
Resolution chapter and in `docs/common/check_resolution`.
...
- **Additional Damage** — the elemental-weapon extra-dice roll (`Successes +
  Crits`), used by the Attack rider phase.
...
write through Conditions. Player-facing rules live in the Conditions material
under `docs/common/conditions`.
...
mutate. Combat never stores HP, mana, toxicity, or effects. (**Afflictions** is
