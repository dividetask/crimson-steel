...
1. **Magical Tier** — pinned as the first nav entry, above the Glossary. A player-facing Explainer chapter (sourced from `docs/common/magical_tier/magical_tier_explainer.md`) introducing magic density and the Tier scale (0–6), magical saturation, and the six attributes.
2. **Glossary** — the default landing pane for `/compendium` (visiting `/compendium` with no sub-view still lands here). The right pane renders the union of every glossary in `docs/common/` (`common_glossary.md`, `dice_resolution/dice_resolution_glossary.md`, `check_resolution/check_resolution_glossary.md`, `conditions/conditions_glossary.md`). Each source becomes its own group with the common glossary listed first; within a group, the source's `##` headings become subsections and each `**Term**: definition` paragraph becomes a definition-list entry.
