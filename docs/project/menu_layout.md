...
1. **Chapter 1 of the player's manual** — pinned as the first nav entry, above the Glossary. Currently **Magical Tier**, introducing magic density and the Tier scale (0–6), magic toxicity, and the six attributes.
2. **Glossary** — the default landing pane for `/compendium` (visiting `/compendium` with no sub-view still lands here). The right pane renders the union of every `*_glossary.md` under `docs/common/`, discovered by directory scan with `common_glossary.md` first. Each source becomes its own group; within a group, `##` headings become subsections and each `**Term**: definition` paragraph becomes a definition-list entry.
3. **Spells** — the Spell List (see `docs/common/ui/abilities_spell_list_stub.md`): a filterable table of every Spell, expanded into its individual Variants — a Tier-axis Spell shows one row per Tier (Heal → Heal Petty Wounds, Heal Lesser Wounds, …), an aspect-axis Spell one row per aspect (Elemental Dart → Fire / Acid / Electricity / Cold Dart). Rows are **sorted by Tier** (lowest first) with no Tier column. Filters: School / Tier / Skill. Casting Skills exclude the Universal Spell Casting Skills (Evocation). Clicking a spell name opens its description (`abilities_spell_detail_stub.md`); clicking its **School** opens the School's description (from `abilities_config.yaml` → Spell Schools) — both in the shared modal.
4. **Classes** — the Class List (see `docs/common/ui/class_list_stub.md`): a card per **base** playable Class (every Class in `creatures_advancement.yaml` not flagged `npc_class: true` and not an archetype — so Ranger and Arcane Trickster are excluded), each showing a short summary. Clicking a card opens the Class's page in the shared modal, laid out like the Classes design document — Proficiencies (Armor / Weapons / Saving Throws), the Skills count, Aligned Skills and All Skills, a level progression table (Mana; the Aligned / Unaligned / Opposed proficiency-rate ranks; per-Tier Spells Known for a Class with an authored spell progression such as the Bard; and the Special Abilities per level, all derived live from the advancement config), and the Class Features with their descriptions. The one-line summary shows only on the list, never on the page.
5. **The remaining chapters of the player's manual**, in chapter order, each labelled with its number — currently Dice Resolution (2), Check Resolution (3), and Conditions (4).
