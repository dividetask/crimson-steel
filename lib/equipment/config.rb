...
module Equipment
  ...
  class Catalog
    ...
    # Types (see Spell-Form Item Types below). It is a list of raw Spell
    # entries (`Abilities.list(type: 'spell')` shape: `name`, `tier`,
    # `items`). Empty (the default) means no generated forms — handcrafted
    # test catalogs built via `Catalog.new(data)` stay exactly as authored.
    def initialize(data = {}, spells: [])
      @data = data || {}
      @spells = spells || []
    end

    # `spells: :auto` (the default for the production catalog) pulls the
    # Spell catalog from Abilities so every eligible Spell gets its item
    # forms. Pass an explicit list (or `[]`) to override.
    def self.load(path = DEFAULT_PATH, spells: :auto)
      data = YAML.safe_load_file(path) || {}
      spells = load_spells if spells == :auto
      new(data, spells: spells)
    end
    ...
    def item_type(name)
      ...
      # Autogenerated Spell-Form Item Types come last, so an explicitly
      # authored entry of the same name always wins.
      if (defn = spell_forms[name])
    ...
    def item_types_in_category(category)
      ...
      spell_forms.each { |name, defn| out << name if defn['category'] == category }
      out
    end
