...
# the way the views expect so the interface can be rebuilt one page
# at a time.
#
# Production rule: nothing in here loads in production. Every
# public method funnels through `gate(empty) { ... }`, which
# returns the empty default unless DummyData is enabled. app.rb
# sets `DummyData.enabled = settings.development?` so prod stays a
# clean slate — the only data the UI sees in prod is whatever's in
...
class DummyData
  ...
  class << self
    ...
    # Each public data method funnels its body through this guard.
    # `empty` is what the method returns when DummyData is off.
    def gate(empty)
      enabled ? yield : empty
    end
  end
  ...
  # Derived methods don't need their own guard — they read from a
  # gated method, so they auto-empty when DummyData is off.
  def self.pcs
