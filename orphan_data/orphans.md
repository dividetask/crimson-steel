...
The enumerated list of valid Bonus / Penalty / Starting modifier types (e.g. Circumstance, and any future named types). The data lives in `orphan_data/modifiers.yaml` until the Modifiers domain is designed.

The dice resolution domain treats modifier keys as opaque strings — it neither owns the type list nor validates names against it. Whoever assembles the modifier dictionary is responsible for using only recognized type names.

**Open questions:**

- Which domain owns the canonical list? (See `domain_index.md` — currently the tentative Modifiers domain.)
