...
RSpec.describe 'Enemy templates: races, clamp, NPC class, loadout', type: :model do
  ...
  describe 'fey monster races removed' do
    it 'sprite/pixie/dryad/brownie are no longer known races' do
      %w[sprite pixie dryad brownie].each do |r|
        expect(Creatures::Races.known?(r)).to be(false), "expected #{r} to be removed"
      end
    end

    it 'keeps the fey root and sidhe (used by NPC stat blocks)' do
      %w[fey sidhe].each do |r|
        expect(Creatures::Races.known?(r)).to be(true), "expected #{r} to remain"
      end
    end
  end
