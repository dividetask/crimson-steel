...
**Visible-to filter respects Public flag.** With one Public Note and one private Note (`public = false`): List Entries with `visible_to = some_creature_id` returns only the Public Note.
...
**Visible-to filter respects owner_id.** With a Note having `owner_id = 7` and `public = false`: List Entries with `visible_to = 7` returns the Note. List Entries with `visible_to = 8` does not.
